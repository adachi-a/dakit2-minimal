<script>
  ...
  import fuseKeys from './dakit_fuse_keys.json';
  import sortNames from './dakit_sort_names.json';
  import configuration from './dakit_itemsjs_configuration.json';
  ...
  <div class="col-md-3">
    ...
    <div class="my-4">
      <ul class="list-group">
        ...
        <button class="list-group-item d-flex align-items-center list-group-item-primary" on:click={()=>
        ...
        {#if !collapsedState[title] || index < bucketThreshold} <!-- render top (bucketThreshold) if collapsed -->
          <li class="list-group-item d-flex align-items-center">
            <input type="checkbox" checked={isChecked(title, bucket.key)} on:change={()=> toggleFilter(title,
            bucket.key)} value={bucket.key} />
            <span class="ms-2">{#if bucket.key}{bucket.key}{:else}N/A{/if}</span>
            <span class="mx-auto" />
          ...
          <button class="list-group-item text-end" on:click={()=>
            toggleCollapse(title)}><small>ほか{details.buckets.length - bucketThreshold}件を表示……</small></button>
          {/if}
      </ul>
    </div>
    {/each}
    ...
    <nav aria-label="Page navigation" class="mt-3 d-flex justify-content-center">
      <ul class="pagination">
        <li class="page-item {currentPage === 1 ? 'disabled' : ''}">
          ...
          <a class="page-link" href="#" on:click={()=> goToPage(1)} aria-disabled={currentPage === 1}><i
              class="bi bi-chevron-bar-left" /></a>
        </li>
        <li class="page-item {currentPage === 1 ? 'disabled' : ''}">
          <!-- svelte-ignore a11y-invalid-attribute -->
          <a class="page-link" href="#" on:click={()=> goToPage(currentPage - 1)} tabindex="-1"
            aria-disabled={currentPage === 1}><i class="bi bi-chevron-left" /></a>
        </li>
        ...
        {/each}
        {#if currentPage < totalPages - maxPageButtons / 2} <li class="page-item disabled"><span
            class="page-link">...</span></li>
          {/if}
          <li class="page-item {currentPage >= totalPages ? 'disabled' : ''}">
            <!-- svelte-ignore a11y-invalid-attribute -->
            <a class="page-link" href="#" on:click={()=> goToPage(currentPage + 1)} aria-disabled={currentPage >=
              totalPages}><i class="bi bi-chevron-right" /></a>
          </li>
          <li class="page-item {currentPage >= totalPages ? 'disabled' : ''}">
            <!-- svelte-ignore a11y-invalid-attribute -->
            <a class="page-link" href="#" on:click={()=> goToPage(totalPages)} aria-disabled={currentPage >=
              totalPages}><i class="bi bi-chevron-bar-right" /></a>
          </li>
      </ul>
    </nav>
    {/if}
